...
| 이름 | 기본값 | 의미 |
| --- | --- | --- |
| `TIME_BUDGET_S` | 2000 | **시간 예산 가드.** 20장 이후 진행 속도로 총 소요를 예측해 예산 초과가 예상되면 남은 장을 경량 모드(사다리 640만, 영문 폴백 생략)로 처리. 타임아웃(2400s) 시 정량 전체 0점을 구조적으로 방지. 개발 PC 2스레드 실측 2438s → 가드로 예산 안쪽 |
